...
async function initializeSheets() {
  ...
  for (const title of sheetsToCreate) {
    if (!existingSheets.includes(title)) {
      ...
      let headers: string[] = [];
      if (title === 'Transactions') headers = ['id', 'date', 'time', 'description', 'amount', 'type', 'category_id', 'user_id', 'observation', 'attachment_url', 'status', 'account', 'created_at'];
...
app.post(["/api/transactions", "/transactions"], async (req, res) => {
  try {
    const { date, time, description, amount, type, category_id, user_id, observation, attachment_url, status, account } = req.body;
    const id = uuidv4();
    const createdAt = new Date().toISOString();
    await appendRow('Transactions!A:M', [
      id, 
      date, 
      time, 
      description, 
...
app.put(["/api/transactions/:id", "/transactions/:id"], async (req, res) => {
  try {
    ...
    const rows = await getRows('Transactions!A:M');
    const rowIndex = rows.findIndex((row, index) => {
      if (index === 0 || !row[0]) return false;
      return row[0].toString().trim().toLowerCase() === id.trim().toLowerCase();
    });
